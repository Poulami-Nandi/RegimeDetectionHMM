...
def base_forecast_close_arima(ohlcv: pd.DataFrame, order=(1, 1, 1)) -> pd.Series:
    """One-step-ahead ARIMA baseline for the Close series (aligned index)."""
...
with st.spinner("Fetching data & detecting regimes…"):
    ...
    # Unchanged pipeline over full history, then align to zoom/base index later
...
# ARIMA baseline on last-N-years OHLCV
...
- **Bias layer** — `vol_span`, `bull_k`, `bear_k_cand`, `bear_k_conf`.
"""
)
